...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      if (k && k.startsWith("gsk_")) setApiKey(k);
      else { localStorage.removeItem("kairos_api_key"); setShowKeySetup(true); }
    } catch {}
  }, []);
  ...
  const saveApiKey = () => {
    ...
    if (!trimmed.startsWith("gsk_")) {
      alert("That doesn't look like a valid Groq API key. It should start with gsk_");
      return;
    }
    ...
    try {
      ...
      const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1500,
          messages: [
            { role: "system", content: SYSTEM_PROMPT + registerContext },
      ...
      setError(`Could not process attendance. Debug: apiKey=${apiKey ? apiKey.slice(0,8)+"..." : "EMPTY"}, url=/api/claude`);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div style={S.keySetupWrap}>
          ...
          <div style={S.keySetupCard}>
            ...
            <p style={S.keySetupText}>
              This app uses Groq AI to process attendance. Groq is completely free — no credit card needed.
            </p>
            <div style={S.keyStep}>1. Go to <span style={S.keyLink}>console.groq.com</span></div>
            <div style={S.keyStep}>2. Sign up for a free account</div>
            <div style={S.keyStep}>3. Click <b>API Keys</b> → <b>Create API Key</b></div>
            <div style={S.keyStep}>4. Copy the key (starts with gsk_...)</div>
